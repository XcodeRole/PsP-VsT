...
void Foam::discreteVelocity::updateGHbarPvol()
{
    ...
    const scalarField &rho = dvm_.rhoVol();
    const vectorField &U = dvm_.Uvol();
    const scalarField &T = dvm_.Tvol();
    const vectorField &q = dvm_.qVol();
    const scalarField &tau = dvm_.tauVol();
    scalar R = dvm_.R().value();
    scalar Pr = dvm_.Pr();
    label D = mesh_.nSolutionD();
    label K = dvm_.KInner();
    for(int internal_id = 0;internal_id<mesh_.nCells();internal_id++){
        scalar relaxFactor;
        // if constexpr (vol){
        ...
        scalar cSqrByRT = magSqr(U[internal_id]-xii)/(R*T[internal_id]);
        
        scalar cqBy5pRT = ((xii - U[internal_id])& q[internal_id])/(5.0*rho[internal_id]*R*T[internal_id]*R*T[internal_id]);    
        scalar gEqBGK = rho[internal_id]/pow(sqrt(2.0*pi*R*T[internal_id]),D)*exp(-cSqrByRT/2.0);
            
        gBarPvol_[internal_id] = (1.0 - relaxFactor)*gTildeVol_[internal_id] + relaxFactor*( 1.0 + (1.0 - Pr)*cqBy5pRT*(cSqrByRT - D - 2.0) )*gEqBGK;
        hBarPvol_[internal_id] = (1.0 - relaxFactor)*hTildeVol_[internal_id] + relaxFactor*( (K + 3.0 - D) + (1.0 - Pr)*cqBy5pRT*((cSqrByRT - D)*(K + 3.0 - D) - 2*K) )*gEqBGK*R*T[internal_id];
    } 
}

void Foam::discreteVelocity::GHbarPvolComm(){
    gBarPvol_.setUpToDate();
    gBarPvol_.storeOldTimes();
    hBarPvol_.setUpToDate();
    hBarPvol_.storeOldTimes();
    forAll(gBarPvol_.BOUNDARY_FIELD_REF, patchi)
    {
        if(gBarPvol_.BOUNDARY_FIELD_REF[patchi].type()!="processor"){

            gBarPvol_.BOUNDARY_FIELD_REF[patchi].initEvaluate(Pstream::defaultCommsType);
            hBarPvol_.BOUNDARY_FIELD_REF[patchi].initEvaluate(Pstream::defaultCommsType);
            gBarPvol_.BOUNDARY_FIELD_REF[patchi].evaluate(Pstream::defaultCommsType);
            hBarPvol_.BOUNDARY_FIELD_REF[patchi].evaluate(Pstream::defaultCommsType);
        }
    }
}
